...
このファイルは実装上の判断と未決事項の記録です。
実装担当が判断したときに追記します。
指示書ファイル自体は編集しません。

## 2026-09-05 日次業務ベータ

- 新しい日の必須確認は7カテゴリ。構成変更は初めて準備する日から適用し、旧台帳の既存日を遡って変更しない。取得状態・本人の確認・タスク結果は別々に保存する。
- ノートは未信頼の資料。明示ACTION行だけを候補化し、出典と会議IDを採用後も保持する。本人の回答なしに採用・完了・再開しない。
- クラウドMCPは明示プロファイルと設定許可の両方が必要。ローカルプロファイルの出力は無制限で、ホストの通信や直接貼付けをdayloopは制御しない。
- 通知の評価・保留・配信・業務の確定を分離する。過去日や過去週の未回答は元の日付のまま保持し、後の通知枠で確認する。配信の成功は既読や業務完了ではない。
- 台帳への保存後にMarkdown出力だけ失敗した場合、CLIは警告、MCPは `warnings[].code=markdown_export_failed` を返す。保存済み操作を失敗扱いにせず、`export` の再試行を案内する。
- Outlookのフィルタ・行取得・型変換に失敗した場合、未制限のItemsへ戻さず取得失敗を報告する。500件を超える場合も黙って切り捨てない。繰り返し予定は `Count` に依存せず列挙する。公式仕様ではStartの昇順ソート→IncludeRecurrences→絞り込みの順を要求し、繰り返し展開後のCountは未定義としている。[Microsoft Learn](https://learn.microsoft.com/en-us/office/vba/api/Outlook.Items.IncludeRecurrences)
- 配布ZIPはレビュー済みのクリーンなコミットから作り、マニフェストとSHA-256を含める。署名・公開Release・企業環境での実行許可は別の受入れである。
