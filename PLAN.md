...
- Task 1 remote CI: follow-up ae5d624 passed both Windows and Linux (run 33958103539).
- Tasks 2–3: implemented. Schema v2 adds seven dated reviews, fetch reports, immutable observations/provenance and weekday routines. CLI and MCP expose review/routine/note/retro operations; malformed/empty arguments are rejected. Local and explicitly enabled GitHub Copilot profiles control response fields independently from source body intake.
- Task 4: implemented for local beta evaluation. The scheduler preserves dated pending work across days/weeks, re-evaluates at later slots, groups catch-up, and never completes business work unattended. Native Win32 notification and explicit HKCU registration replace the previous PowerShell identity / script fallback. Doctor is read-only unless an explicit notification test is requested. Package generation records source commit and SHA-256 without publication/signing.
- Final review: a fresh read-only Sol review accepted the fixes for COM scope/truncation failure, cross-date pending reminders, and post-commit Markdown warnings. 116 local tests passed, all-target Clippy passed, and release build passed. Three persona CLI scenarios and 12 real-executable stdio assertions passed with isolated data. Win32 notification returned API acceptance; visible delivery/reading remains unverified.
